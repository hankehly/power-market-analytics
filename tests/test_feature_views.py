...
def test_fields_carry_the_marts_types_and_categorical_tags():
    calendar = {field.name: field for field in views.FTR_DAY_CALENDAR.schema}
    ...
    # Feast lists the entity join keys in the schema next to the features.
    assert "available_at" not in calendar
    assert {"area_code", "trade_date_key"} <= set(calendar)
