...
class TestJoinLag:
    def test_one_day_lag_attaches_previous_day_same_time_code(self):
        df = prices([D1, D2, D3])
        out = join_lag(df, df, days=1, name="lag_1d")
    ...
    def test_lag_length_is_in_calendar_days(self):
        df = prices([D1, D2, D3])
        out = join_lag(df, df, days=2, name="lag_2d")
    ...
    def test_left_frame_may_differ_from_price_source(self):
        ...
        out = join_lag(left, history, days=1, name="lag_1d")
    ...
    def test_source_price_frame_is_not_mutated(self):
        df = prices([D1, D2])
        before = df.copy()
        join_lag(df, df, days=1, name="lag_1d")
