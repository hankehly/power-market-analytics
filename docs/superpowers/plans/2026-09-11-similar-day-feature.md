...
- [x] **Step 4c: Cutoff-based rerun** — `python scripts/fit_similar_day.py --area tokyo`, run `d57d9bc628d443d694090e23099b67b8`: 388 fits at cutoffs every 7 days from 2019-04-03 00:00 (when the first scorable day's load was public) to 2026-09-02 00:00, each on the pairs public by its cutoff — the cutoff sits two days after the fit's last target day before 2022-04 (the yearly files) and one day after from then on (the daily files); 2,714 days scored (2019-04-04 to 2026-09-07), 130,272 rows written; 2,712 scored days have a known load and are checked; about 9 minutes. `dbt build --select stg_ml__similar_day ftr_period_similar_day`: 35 nodes pass (the unit test included). The table was dropped first, its schema having changed again. Weights over the fits: the 388 fits use 61 to 165,371 pairs and reach RMSE 0.032 to 0.087; from 2022 on the weights barely move — `temperature` 0.42 to 0.47, `holiday_degree` 0.45 to 0.49, `calendar_days` 0.04 to 0.06, `days_since_holiday` 0.03 to 0.05, `humidity`, `rain` and `days_until_holiday` 0.00 — while the 2019 fits on a few weeks of pairs range wider (`temperature` 0.21 to 0.52, `holiday_degree` 0.33 to 0.69, `days_since_holiday` up to 0.31). The last fit (cutoff 2026-09-02) is `calendar_days=0.045, temperature=0.470, holiday_degree=0.449, days_since_holiday=0.036`. Retrieval check over the 2,712 scored days with a known load, all out of sample: mean load difference of the selected day 0.052 against 0.077 for D − 364 and 0.021 for the oracle; the selected day beats D − 364 on 56.8 % of days (the frozen fit's in-sample figures on the reference run were 0.048 / 0.075 / 0.021 and 59.1 %). Backtest on this feature, run `20e483c32d024438bf85d3217b1bbae7` (729 days, 34,954 periods, one skipped day): against the reference run `008868fe…` MAE 585,362 → 588,051 kWh (+0.5 %), MAPE 3.61 → 3.62 %; by day type weekdays −0.1 %, weekends +0.7 %, holidays +3.0 %; by day part overnight +0.1 %, morning −0.9 %, daytime +1.0 %, evening −0.0 %; candidate lower on 356 of 729 days, mean daily-MAE difference +2,692 kWh, 95 % bootstrap CI over days [−537, +5,821]. Against the frozen-fit write-back run `929ec177…` MAE 585,065 → 588,051 kWh (+0.5 %); holidays +3.1 %, weekends +0.7 %, weekdays −0.0 %; daytime +1.1 %, morning −0.8 %; candidate lower on 352 of 729 days, mean daily-MAE difference +2,988 kWh, 95 % bootstrap CI over days [−213, +6,120]. The two runs share every input but the weights: the frozen run scored every row, training rows included, with the one fit through 2024-08-16, the walk-forward run scored each row with the latest fit before its issue time.
...
## Follow-up, 2026-09-12: the fit window slides

The researcher's call on the first run's fits table (`d57d9bc6…`): every fit took every pair
back to 2019, so `n_targets` grew from 267 to 2,711 and the last fit saw 165,371 pairs. The
weights settled from 2022 on because each new year was a shrinking share of the fit, which
says nothing about drift. Now a fit at cutoff C sees the pairs of the `--fit-window-days`
days before C's day (default 730, `DEFAULT_TRAIN_WINDOW_DAYS`, the LightGBM strategies'
training window: the feature and the model rest on the same span of history) whose target
load was public by C. The first fit still runs at the first instant eight pairs lie inside
