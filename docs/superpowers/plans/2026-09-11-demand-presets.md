...
- [ ] **Step 1: Old runs** — from the main checkout in the container (`docker compose … exec -T -w /workspace -e PYTHONPATH=/workspace devcontainer python scripts/demand_backtest.py --strategy lightgbm_msm_popw_daytype --area tokyo --start-date 2024-08-18 --end-date 2026-08-17 --train-start 2023-01-01 --importance-repeats 1`), then the same for `lightgbm_msm_popw_daytype_simday`; note the run ids.
- [ ] **Step 2: New runs** — the same two commands from the worktree (the Global Constraints command), one at a time.
- [ ] **Step 3: Compare** — through the warehouse (`pma_ml.demand_forecast`, `pma_ml.demand_forecast_contribution`): row counts and skipped days equal per pair; `max(abs(new.forecast_demand_kwh - old.forecast_demand_kwh))` over the joined periods; `max(abs(feature_value))` difference per component; the contributions' difference. Expect 0. A difference is a feature-definition bug to fix before the PR.
- [ ] **Step 4: Docs** — CLAUDE.md: the demand backtest command (presets, `--add/--drop/--name`, the family), the demand task bullet (the strategy descriptions become preset descriptions; the deleted classes, frames and loaders), the Feast bullet ("the demand strategies until PR 6 do not"); spec §11 PR 6 done, §7 the family's interim shape; `docs/research/demand/README.md` unchanged names; this plan's boxes.
- [ ] **Step 5: PR** — `feat(demand): presets over Feast replace the four LightGBM strategy classes`, Why / What / Proof (the comparison table), labels `enhancement` + `documentation`, assignee; the Codex loop; report ready.
