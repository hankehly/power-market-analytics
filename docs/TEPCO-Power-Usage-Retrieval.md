...
after 2022-04. Its `hour_of_day` references `dim_hour`, the 24-row shrunken
rollup of `dim_half_hour`, so the two facts drill across:
`fct_area_demand_generation_actual` summed per `dim_half_hour.hour_of_day`
is the hourly kWh comparable to this fact's `demand_kwh` (the comparison in [§5](#5-comparison-with-the-a-1-series-2022-04-01--2026-08-27-38621-hours)
