...
| 5 | The day before delivery: the forecast for D-1 from 13:00 JST, leads 16–27 of the run already downloaded. Observations end at D-2 and the forecast covers D, so D-1 is unseen. | Twelve more leads in the MSM extractor. The GRIB files are deleted after extraction and the cached extracts hold leads 28–51 only, so the history needs every FH16-33 file downloaded again with `--force`, one of the three files per day, or the feature starts with new data. | — | Ready |
