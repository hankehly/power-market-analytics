...
| R-004 | [Year-ago load on the prior-year reference date](research/demand/R-004-prior-year-load-lag.md) | In progress | E-001 (triggered by O-002 / O-003): `lightgbm_msm_popw_daytype_lag1y` = the baseline + `lag_1y_demand_kwh`, the でんき予報 hourly load on `dim_date.prior_year_reference_date` per period; candidate run started 2026-08-31 on the matched 729-day window against run `0a6b8a55…` — results pending. |
