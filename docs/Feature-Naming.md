...
| `… by col` | The window runs over the days with the delivery day's value of `col`. With a `step` the terms keep that spacing, and a term whose day has another value is skipped, so the window reaches further back, up to the bound the description gives. | `ROLLING_MEAN(demand_kwh, gap=2d, window=4) by day_type`, `ROLLING_MEAN(demand_kwh, gap=7d, window=4, step=7d) by day_type` |
