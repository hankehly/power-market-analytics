...
FTR_PERIOD_SIMILAR_DAY_SOURCE = SparkSource(
    ...
    description="The load of a learned similar day one year earlier for every delivery period, the demand similar-day feature (research demand/R-004 E-002), as scripts/fit_similar_day.py scored and wrote it to pma_ml.similar_day: the script fits the seven weights of tasks/demand/similar_day.py's distance, picks for every delivery day D the nearest day in D - 364 +- 30 (days from D - 364, the 24-hour RMSE of D's population-weighted MSM forecast against the candidate's population-weighted observation for temperature, humidity and rain, the differences in days since and until a named holiday and in holiday degree), and halves the chosen day's hourly load over the hour containing the period. The chosen day, its lag, its distance and the candidate count sit next to the feature, untagged. One row per scoring run: the as-of join takes the newest run available at the issue time, and among rows tied on available_at the newest published wins, so a refit-and-rescore replaces the feature wherever it scored. available_at is the day's MSM forecast vintage's; every candidate is at least 334 days older. Grain: area_code x trade_date x time_code x similar_day_run_id.",
)
