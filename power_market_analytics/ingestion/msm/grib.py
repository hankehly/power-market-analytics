"""Decode one archive member into station records with ecCodes.

An archive member is a plain concatenation of GRIB2 messages, one per (element,
forecast hour), each carrying a full 505 x 481 grid. The decoder walks those
messages, identifies each by its metadata, samples the grid at the point nearest
every station and returns one record per station and forecast hour, with the
pipeline's unit conversions applied.

It is deliberately strict — a silently short or mislabeled extract would become a
silently wrong forecast feature downstream:

* the run stamped on every message must equal the reference time the caller asked
  for, the edition must be GRIB2 and the data must be operational
  (``productionStatusOfProcessedData = 0``);
* an element's ``typeOfFirstFixedSurface`` must match
  :data:`~power_market_analytics.ingestion.msm.elements.MSM_SURFACE_ELEMENTS`, and
  the grid must scan i-fastest (``jPointsAreConsecutive = 0``);
* every configured element must be present for every used forecast hour — a
  missing message raises rather than yielding a record with a hole. Only a bitmap
  (an explicitly missing *value*) produces ``None``.

Messages the pipeline does not use — an unconfigured parameter, or a forecast hour
outside ``source_file.leads_used`` — are skipped, and only one message's values are
held in memory at a time. JMA packs many fields into one message envelope, so
ecCodes needs ``codes_grib_multi_support_on()`` (process-global, re-asserted per
call) or it yields only the first field.
"""
...
from power_market_analytics.ingestion.msm.errors import MsmExtractError
from power_market_analytics.ingestion.msm.grid import (
    MsmGrid,
    SelectedGridPoint,
    select_grid_point,
)
