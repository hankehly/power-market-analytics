...
metastore from ``SPARK_CONF_DIR``:
    ...
    python scripts/load_kansai_area_demand_generation.py
"""
...
from power_market_analytics.ingestion.tso.kansai.area_demand_generation import KansaiAreaCsvLoader
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--schema",
        type=Path,
        default=REPO_ROOT / "conf/schemas/kansai_area_demand_generation_actual.yaml",
        help="Path to the YAML schema definition.",
    )
    parser.add_argument(
        "--data",
        type=Path,
        default=REPO_ROOT / "data/kansai/area_demand_generation/csv",
        help="CSV file, directory of CSV files, or glob pattern to load.",
    )
    parser.add_argument(
        "--table",
        default="pma_raw.kansai_area_demand_generation_actual",
        help="Destination table (database.table).",
    )
    args = parser.parse_args(argv)

    schema = CsvTableSchema.from_yaml(args.schema)
    loader = KansaiAreaCsvLoader(schema=schema, filepath=args.data, table=args.table)
    n_rows = loader.load()
