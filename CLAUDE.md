...
- `just python scripts/demand_backtest.py --strategy lightgbm_msm_popw_daytype --area tokyo` —
  ...
  run 2026-09-06, decisions pending): `lightgbm_msm_popw_daytype_simday_holidaydegree` = the
  ...
  every day part / day type / season worse); areas: `tokyo`,
...
- Demand task (`tasks/demand/`): at 09:30 JST on D-1 forecast the 48 half-hourly `demand_kwh`
  ...
  E-003, run 2026-09-06, decisions pending): `lightgbm_msm_popw_daytype_simday_holidaydegree`
  ...
  `f7153839…`: MAE +6.5 %, CI excludes zero, every day part / day type / season worse); each
  with its own eval-set class (`_calendar_subset_schema` drops the other calendar columns from
  the ten-feature schema), the same constructor and inputs, wired through the same branch.
